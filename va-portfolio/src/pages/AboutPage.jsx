...
const Icons = {
  ...
  Calendly: () => (
    <svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor">
      <path d="M19 3h-1V1h-2v2H8V1H6v2H5C3.346 3 2 4.346 2 6v14c0 1.654 1.346 3 3 3h14c1.654 0 3-1.346 3-3V6c0-1.654-1.346-3-3-3zm1 17c0 .551-.449 1-1 1H5c-.551 0-1-.449-1-1v-9h16v9zm0-11H4V6c0-.551.449-1 1-1h1v2h2V5h8v2h2V5h1c.551 0 1 .449 1 1v3zm-4 5h-2v-2h2v2zm-4 0H10v-2h2v2zm4 3h-2v-2h2v2zm-4 3H10v-2h2v2zm-4-3H6v-2h2v2zm0-3H6v-2h2v2z" />
    </svg>
  ),
  GSuite: () => (
...
/* Stronger animation — fires on every scroll in & out */
...
/*
  GlintSocialButton
  ─────────────────
  A thin 1px border sits permanently at low opacity.
  A sharp, narrow light streak — like sunlight catching a coin edge —
  sweeps slowly around the border once every ~4s, then pauses briefly
  before going again. It's a conic-gradient rotated via rAF, but the
  arc is kept very tight (only ~40deg wide) so it reads as a glint
  rather than a glow. No color, just white light.
  Clicking scrolls to the contact section.
*/
function GlintSocialButton({ title, Icon, ledColor, delay, onClick }) {
  ...
  React.useEffect(() => {
    const CYCLE = 4200; /* ms for one full sweep */
    const PAUSE = 1800; /* ms idle after each sweep */
    ...
    const timer = setTimeout(() => {
      function tick(ts) {
        ...
        if (elapsed < CYCLE) {
          /* sweeping phase — map elapsed → 0..360 */
          setAngle((elapsed / CYCLE) * 360);
        }
        /* during PAUSE phase we just hold the angle at 360 (off-screen) */
  ...
  /*
    The ring is built from two absolutely-positioned divs:
    1. Outer — the conic-gradient background (full size, border-radius matches button)
    2. Inner — solid dark fill, inset by 1px, masks the centre

    The conic has a razor-thin bright arc:
      0-15deg  : bright white glint peak
      15-40deg : rapid fade to nothing
      40-360deg: fully transparent (invisible border)

    So 320deg of the border is invisible, 40deg is the glint.
  */
  const conicGrad = `conic-gradient(
    ...
    rgba(255,255,255,0.7)  20deg,
    rgba(255,255,255,0.0)  38deg,
    rgba(255,255,255,0.0) 360deg
  )`;

  /* base border always present but very dim */
  const baseBorder = hovered
  ...
  return (
    ...
    >
      {/* Glint ring — conic sits as background, inner tile punches a hole */}
      <div
        style={{
          position: "absolute",
          inset: 0,
      ...
      {/* Inner tile — 1px inset exposes the glint ring */}
      <div
        style={{
          position: "absolute",
          inset: "1px",
          borderRadius: "12px",
          /* static dim border so the button has shape even when glint is behind */
...
export default function AboutPage() {
  return (
    ...
    >
      ...
      {/* Base indigo gradient — matches intro page atmosphere */}
      <div
      ...
      <div className="absolute inset-0 grid grid-cols-12">
        ...
        <div className="col-span-5 relative h-full overflow-hidden">
          ...
          {/* Name — bottom left, below her face */}
          <motion.div
        ...
        >
          ...
          >
            <div>
              ...
              <p
                style={{ fontSize: "13px", color: "#52525b", marginTop: "2px" }}
              >
                Nigeria
              </p>
            </div>
